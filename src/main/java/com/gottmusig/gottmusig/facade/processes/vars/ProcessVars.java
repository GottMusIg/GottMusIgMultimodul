package com.gottmusig.gottmusig.facade.processes.vars;

public final class ProcessVars {

    //PROCESS IDs
    public static final String ITEM_RANKING_SIMULATION_ID = "itemSimulationProcess";
    public static final String MAIN_PAGE_DPS_SIMULATION_ID = "simulateMainPageDpsProcessId";


    //COMMON
    public static final String SIMULATIONCRAFT = "simulationCraft";
    public static final String Blizzard_ITEMS = "blizzardItems";
    public static final String SIMC_VERSION = "simcVersion";

    public static final String WAS_ALREADY_STARTED_WITH_WANTED_VERSION = "wasAlreadyStartedWithVersion";

    //ITEM RANKING



    public static final String WOW_HEAD_OPTION_LIST = "wowHeadOptionsList";
    public static final String WOW_HEAD_OPTION = "wowHeadOption"
            + "";
    public static final String SIMULATION_RESULT_LIST = "simulationResultList";

    public static final String WOW_HEAD_ITEMS = "wowHeadItemIds";
    public static final String WOW_HEAD_ITEM_ID = "wowHeadItemId";






}
